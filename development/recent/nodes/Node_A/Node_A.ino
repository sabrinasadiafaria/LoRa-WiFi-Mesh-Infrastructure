// ===========================================================================
//  PHASE 9 (FINAL)  -  INTEGRATION BUILD FOR DEMO DAY  -  NODE A
...
//
//  This is the same sketch as phase 8, retitled as the canonical
//  demo-day build. Everything that earlier phases added lives in this
//  one file: phase 1 shared core, phase 3 edge-triggered SOS, phase 4
//  portal + rescue reports, phase 5 unified self-healing routing,
//  phase 6 reliability fixes (heap/CRC/loop-time guards), phase 7
//  range + GPS, phase 8 ROVER: relay.
//
//  Earlier phase folders are still there for diffs and rollback; this
//  file is what you flash on demo day.
//
//  WHY THIS PHASE EXISTS - nodes would not link up outdoors, and the GPS
//  looked dead. Two real bugs were behind it:
//
//  1. SPREADING FACTOR WAS TUNED FOR A DESK, NOT A FIELD.
//     SF7 was chosen back in Phase 3 from bench measurements taken with the
//     nodes ~2 m apart (RSSI -62..-71 dBm). SF7 has the WORST sensitivity of
//     the usable settings: -123 dBm. Outdoors at distance that margin is
//     gone. Now SF9 (-129 dBm) - 6 dB better, roughly DOUBLE the range.
//
//  2. THE AIRTIME WAIT WAS A HARDCODED CONSTANT (a latent bug).
//     After endPacket(true) the sketch waited a fixed 420 ms before handing
//     the radio back to parsePacket(). That was sized for SF7, whose biggest
//     frame is ~318 ms. At SF9 a full frame needs ~1005 ms - so the wait
//     would have expired MID-TRANSMISSION and parsePacket() would have
//     yanked the radio out of TX, corrupting every large packet. Airtime is
//     now COMPUTED per frame from its length and LORA_SF (loraAirtimeMs),
//     so the spreading factor can be changed freely without breaking TX.
//
//  3. GPS SERIAL BUFFER TOO SMALL.
//     Serial2's default RX buffer is 256 bytes. A NEO-M8N with several
//     constellations emits more than that per second in bursts; when loop()
//     was briefly busy (an SF9 transmit is ~370 ms) the buffer overran and
//     NMEA sentences arrived truncated - looking exactly like a dead GPS.
//     Now 1 KB, set before Serial2.begin().
//
//  Beacon rates were slowed to pay for SF9's longer airtime (HB 15 s,
//  RT 30 s, GPS 45 s) with timeouts scaled to match, keeping the channel
//  around 18% busy with four nodes.
//
//  ALSO IN THIS PHASE
//    * Redesigned OLED - 5 organised pages, including a LINKS page showing
//      signal strength per peer, which is the page to watch during a range
//      test. Every page is marked n/5.
//    * Redesigned captive portal - modern responsive UI, live peer table with
//      signal bars, position card with source badges, toast notifications,
//      collapsible diagnostics. Polls /api/status every 4 s.
//
//  TUNING FOR YOUR SITE
//    Indoors / short range, want speed:   LORA_SF 7
//    Mixed indoor+outdoor (default):      LORA_SF 9
//    Maximum range, slow:                 LORA_SF 10 or 11
...
//           v = toggle rx log   N = raw NMEA dump   R = rescan   h = help
//
//  (phase 8 rover support - relaying ROVER: telemetry - is part of the
//  base forwarding logic now, not an "addition".)
// ===========================================================================
...
// -------------------------------- timing ----------------------------------
#define HB_INTERVAL_MS       15000UL   // slower: SF9 packets are ~3x longer on air
#define HB_JITTER_MS          3000UL
#define GPS_INTERVAL_MS      45000UL
#define GPS_JITTER_MS         4000UL
#define RT_INTERVAL_MS       30000UL   // routing advertisement period
#define RT_JITTER_MS          4000UL
#define ROUTE_TIMEOUT_MS    120000UL   // ~4 missed adverts -> route invalid
#define NEIGHBOR_TIMEOUT_MS  75000UL   // ~5 missed heartbeats at the slower rate
...
#define TX_AIRTIME_MARGIN_MS    80UL   // safety added to the COMPUTED airtime
...
uint32_t txAirtimeMs = 500;       // computed per frame by loraAirtimeMs()
...
#define RADIO_WEDGE_MS      120000UL   // ~8 missed heartbeats at the slower rate
...
// How long will a frame of `len` bytes actually be on the air, in ms?
//
// This REPLACES the old fixed TX_AIRTIME_MS constant, which was a latent bug:
// it was tuned to 420 ms for SF7 (max frame ~318 ms). At SF9 the same frame
// takes ~1005 ms, so the fixed wait would have expired mid-transmission and
// parsePacket() would have yanked the radio out of TX - corrupting every
// large packet. Now the wait follows LORA_SF automatically, so the spreading
// factor can be changed for range without silently breaking transmission.
//
// Standard Semtech SX1276/78 time-on-air formula.
uint32_t loraAirtimeMs(uint16_t len) {
  const double tSym = (double)(1UL << LORA_SF) * 1000.0 / (double)LORA_BW;
  const uint8_t DE  = (tSym > 16.0) ? 1 : 0;   // low-data-rate optimise
  const uint8_t CR  = LORA_CR - 4;             // 1..4
  double tPre = ((double)LORA_PREAMBLE + 4.25) * tSym;

  long num = 8L * (long)len - 4L * LORA_SF + 28 + 16;   // explicit header -> no -20H
...
// endPacket(true) we wait exactly as long as THIS frame needs on the air -
// loraAirtimeMs() computes it from the length and LORA_SF - and then hand the
// radio back to parsePacket(), which re-arms RX on its next call.
void radioService() {
  // ---- finish an in-flight transmission -----------------------------------
  if (txInFlight) {
    if (millis() - txStart < txAirtimeMs) return;     // still on air
  ...
  txInFlight  = true;
  txStart     = millis();
  // Wait for THIS frame's real on-air time, not a fixed guess. At SF9 a full
  // frame needs ~1005 ms; the old fixed 420 ms would have cut it off.
...
// Structured status for the portal. Peers are a proper JSON array now
// (rssi/snr/age/position per node) so the page can render signal bars and a
// live peer table instead of a pre-formatted blob of HTML from the firmware.
void handleStatus() {
  double lat = 0, lon = 0;
  uint32_t ageMs = 0;
  uint8_t src = locBest(lat, lon, ageMs);

  static char peers[600];
  peers[0] = '[';
  peers[1] = '\0';
  size_t used = 1;
  bool first = true;
  for (uint8_t i = 0; i < MAX_NEIGHBORS; i++) {
    ...
    int n = snprintf(row, sizeof(row),
             "%s{\"id\":\"%s\",\"up\":%d,\"rssi\":%d,\"snr\":%.1f,\"age\":%lu,"
             "\"hasloc\":%d,\"lat\":%.6f,\"lon\":%.6f,\"locsrc\":%u}",
             first ? "" : ",",
             neighbors[i].id, neighbors[i].active ? 1 : 0,
             neighbors[i].rssi, (double)neighbors[i].snr,
             (unsigned long)((millis() - neighbors[i].lastSeen) / 1000UL),
             neighbors[i].hasLoc ? 1 : 0,
             neighbors[i].lat, neighbors[i].lon,
             (unsigned)neighbors[i].locSrc);
    if (n < 0 || used + (size_t)n + 2 >= sizeof(peers)) break;
    memcpy(peers + used, row, (size_t)n);
    used += (size_t)n;
    peers[used] = '\0';
    first = false;
  }
  if (used + 2 < sizeof(peers)) { peers[used++] = ']'; peers[used] = '\0'; }

  static char routesJson[400];
  routesJson[0] = '[';
  routesJson[1] = '\0';
  size_t ru = 1;
  bool rfirst = true;
  for (uint8_t i = 0; i < MAX_ROUTES; i++) {
    ...
    char row[100];
    int n = snprintf(row, sizeof(row),
             "%s{\"dest\":\"%s\",\"via\":\"%s\",\"hops\":%u,\"valid\":%d}",
             rfirst ? "" : ",", routes[i].dest, routes[i].nextHop,
             (unsigned)routes[i].hops, routes[i].valid ? 1 : 0);
    if (n < 0 || ru + (size_t)n + 2 >= sizeof(routesJson)) break;
    memcpy(routesJson + ru, row, (size_t)n);
    ru += (size_t)n;
    routesJson[ru] = '\0';
    rfirst = false;
  }
  if (ru + 2 < sizeof(routesJson)) { routesJson[ru++] = ']'; routesJson[ru] = '\0'; }

  static char json[1800];   // peers(600) + routes(400) + fixed(~300), with margin -
                             // a truncated response would be invalid JSON and
                             // the portal would show "connection lost"
  snprintf(json, sizeof(json),
           "{\"id\":\"%s\",\"fw\":%u,\"sf\":%d,\"lora\":%d,"
           "\"lat\":%.6f,\"lon\":%.6f,\"src\":%u,\"age\":%lu,"
           "\"gpsfix\":%d,\"sats\":%d,\"nmea\":%lu,"
           "\"heap\":%lu,\"up\":%lu,\"wifi\":%d,"
           ...
           gpsHasFix ? 1 : 0, gpsSats, (unsigned long)gpsSentences,
...
void handleOneRx() {
  ...
  bool forwardable = !strcmp(p.type, "DATA")   || !strcmp(p.type, "SOS")    ||
                     ...
                     !strcmp(p.type, "ROVER");   // Phase 8: the Rover's telemetry
  ...
  if (strcmp(p.type, "HB") == 0) {
    ...
    // Phase 8: the mobile Rover's telemetry - mode/obstacle/battery. This
    // node doesn't act on it, only relays it, the same reason RPT is
    // relayed below: the Rover may be reachable only through here (that's
    // the whole point of a "mobile relay" - it can be the one out of range).
    Serial.printf("[rover] %s : %s  (rssi %d)\n", p.src, p.payload, p.rssi);
    if (p.ttl > 0) {
      char frame[MAX_PACKET_LEN];
...
// ===========================================================================
//  OLED PAGES  (Phase 7 redesign)
//
//  5 rows x ~21 characters. Reorganised around what you actually need while
//  walking around with the thing:
//
//    0 OVERVIEW   who am I, how many peers, where am I, am I healthy
//    1 LINKS      signal strength per peer      <- the range-test page
//    2 ROUTES     who I can reach and via whom
//    3 POSITIONS  where the other nodes are
//    4 GPS/TEAM   GPS health and team status
//
//  Every page carries a "n/5" marker so you know where you are in the cycle,
//  and every page is built into one char[5][26] then handed to oledPush(),
//  which skips the redraw when nothing changed.
// ===========================================================================

// Four-level signal bar - readable at a glance while moving.
...
void drawPage0() {
  ...
  // Rounded so oledPush()'s change-detection can actually skip most seconds
  // instead of redrawing because a counter ticked by one.
...
void setup() {
  ...
  // STEP 2: GPS serial
  // Bigger RX buffer than the 256-byte default. A NEO-M8N with several
  // constellations enabled emits well over 256 bytes per second in bursts;
  // if loop() is briefly busy (a LoRa transmit at SF9 is ~370 ms) the default
  // buffer overruns and NMEA sentences arrive truncated - which looks exactly
  // like "the GPS isn't working" even though the module is fine.
