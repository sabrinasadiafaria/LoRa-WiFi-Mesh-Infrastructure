// ===========================================================================
//  PHASE 5  -  COMMAND CENTRE LINK  -  NODE B
...
//
//  Everything Phase 4 did (multi-hop mesh, hybrid GPS, captive portal, SOS,
//  rescue reports, team status), plus the Raspberry Pi command-centre link:
//
//    * The Pi runs its own SX1278 as mesh node "PI" (development/phase 5/pi/).
//      Nodes reach it directly if in range, or multi-hop (C -> B -> A -> PI).
//      GPS / SOS / RPT / STAT already flow to it with no node change.
//    * NEW: CMD packets from the Pi are handled here -
//        WHERE  -> reply with a fresh GPS packet
//        PING   -> reply "pong"
//        SOS    -> raise an SOS on this node remotely
//        SOSCLR -> clear this node's alert
//
//  Phase-4 recap of the field-communication layer:
//
//    * SOS ALERT - hardware button OR portal button OR serial. Broadcasts the
//      node's REAL position (from locBest - phone or GPS module) as a 3x
//      non-blocking burst, forwarded across the whole mesh. Every node in
//      range shows a full-screen takeover that CLEARS ITSELF after 60 s, or
//      on a button press. Acknowledgements flow back so the victim's screen
//      shows "heard by: B,C".
//    * QUICK RESCUE REPORTS - Victim Found / Medical / Blocked / Danger,
//      one tap on the portal, tagged with position and team.
//    * TEAM STATUS - Available / Searching / Need Assistance / Emergency,
//      shown on every node's status page and in the portal.
//    * DIRECTED TEXT MESSAGING - already present since Phase 3 as the DATA
//      packet (serial a/b/c, multi-hop).
//
//  The old root phase 11 SOS code is the thing this replaces. Its bugs, fixed
//  here: the alert screen never cleared; coordinates were hardcoded; the 3x
//  burst blocked the loop with delay(500); the button re-fired continuously
//  while held.
//
//  SOS BUTTON: momentary switch from GPIO 4 to GND.
//     tap           -> raise an SOS  (or dismiss a received alert)
//     hold >1.5 s    -> dismiss any alert
//
//  Serial:  S=SOS  C=clear  1-4=reports  5-8=status  (h for the full list)
//
//  WHY THE PORTAL HAS A MANUAL COORDINATE BOX: browsers only allow
//  navigator.geolocation on a SECURE (HTTPS) origin. A captive portal is
//  plain HTTP, so phones usually refuse "Use Phone GPS". Manual entry always
//  works.
// ===========================================================================
